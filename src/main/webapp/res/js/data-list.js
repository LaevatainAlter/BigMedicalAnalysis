angular.module('homeApp').controller('listCtrl', function($scope, $http){
	...
	$scope.toFilterList = function (filterList) {
		$http({
			method: 'GET',
			params: {
			...
			$scope.historyList = req.data.historyList;
			var myRe = new RegExp(filterList);
			var historyStore = [];
			historyStore = $scope.historyList.filter(function(item){
				return myRe.test(item.historyName) || myRe.test(item.historyTime);
			});
	...
	$scope.toReturn = function () {
		...
		d3.select('#data-show-pie .data-show-chart svg').remove();
		$scope.getHistoryList();
		datasetOrigin1 = [], datasetOrigin2 = [];
		$scope.filterParam1 = 10;
		$scope.circleR1 = 4;
		$scope.grid1 = true;
		$scope.assist1 = true;
